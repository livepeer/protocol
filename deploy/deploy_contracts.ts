...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (isArbitrumNetwork(hre.network.name)) {
    ...
    const livepeerTokenAddr = deploymentExport.contracts.LivepeerToken.address;
  ...
  // governor
  const governor = await deploy("contracts/governance/Governor.sol:Governor", {
    from: deployer,
    args: [],
    log: true,
  });
  ...
  const transferOwnershipUpdate = {
    ...
    data: [
      Governor.interface.encodeFunctionData("transferOwnership", [
        isProdNetwork(hre.network.name) ? config.governor.owner : deployer,
      ]),
    ],
    nonce: 0,
  };
