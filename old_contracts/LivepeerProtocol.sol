...
contract LivepeerProtocol {
    ...
    modifier updateStakesWithRewards() {
        ...
        delegators[msg.sender].lastStateTransitionRound = Rounds.currentRound(roundLength);
    ...
    modifier currentRoundInitialized() {
        if (lastInitializedRound != Rounds.currentRound(roundLength)) throw;
        _;
    }
    ...
    function LivepeerProtocol(uint64 _n, uint256 _roundLength, uint256 _cyclesPerRound) {
        ...
        lastInitializedRound = Rounds.currentRound(roundLength);
    ...
    function delegatorStatus(address _delegator) constant returns (DelegatorStatus) {
        ...
        } else if (delegators[_delegator].roundStart > Rounds.currentRound(roundLength)) {
    ...
    function unbondingPeriodOver(address _delegator) constant returns (bool) {
        ...
        if (delegators[_delegator].withdrawRound > 0) {
            ...
            return Rounds.currentRound(roundLength) >= delegators[_delegator].withdrawRound;
        } else if (delegators[_delegator].transcoderAddress != address(0)
                   && transcoders[delegators[_delegator].transcoderAddress].delegatorWithdrawRound > 0) {
            // Transcoder resigned
            return Rounds.currentRound(roundLength) >= transcoders[delegators[_delegator].transcoderAddress].delegatorWithdrawRound;
        } else {
    ...
    function bond(uint _amount, address _to) currentRoundInitialized updateStakesWithRewards returns (bool) {
        ...
        if (transcoders[msg.sender].active == true && _to == msg.sender) {
            ...
            if (del.initialized == false ||
                ...
                del.roundStart = Rounds.currentRound(roundLength).add(1);
            }
    ...
    function unbond() currentRoundInitialized updateStakesWithRewards returns (bool) {
        ...
        delegators[msg.sender].withdrawRound = Rounds.currentRound(roundLength).add(unbondingPeriod);
    ...
    function reward() currentRoundInitialized returns (bool) {
        ...
        transcoders[msg.sender].rewardRound = Rounds.currentRound(roundLength);
        ...
        uint256[2] rewardMultiplier = rewardMultiplierPerTranscoderAndRound[msg.sender][Rounds.currentRound(roundLength)];
        ...
        rewardMultiplierPerTranscoderAndRound[msg.sender][Rounds.currentRound(roundLength)] = rewardMultiplier;
    ...
    function resignAsTranscoder() currentRoundInitialized returns (bool) {
        ...
        transcoders[msg.sender].delegatorWithdrawRound = Rounds.currentRound(roundLength).add(unbondingPeriod);
    ...
    function initializeRound() returns (bool) {
        ...
        if (lastInitializedRound == Rounds.currentRound(roundLength)) return false;
    ...
    function validRewardTimeWindow(address _transcoder) constant returns (bool) {
        ...
        if (transcoders[_transcoder].rewardRound == Rounds.currentRound(roundLength) && transcoders[_transcoder].rewardCycle == cycleNum()) return false;
    ...
    function delegatorRewards(address _delegator) internal constant returns (uint256) {
        ...
        if (delegators[_delegator].transcoderAddress != address(0)) {
            ...
            for (uint256 i = delegators[_delegator].lastStateTransitionRound; i <= Rounds.currentRound(roundLength); i++) {
