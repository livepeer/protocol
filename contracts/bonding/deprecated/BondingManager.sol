...
contract BondingManager is ManagerProxyTarget, IBondingManager {
    ...
    function slashTranscoder(
        ...
        uint256 _finderFee
    ) external override whenSystemNotPaused onlyVerifier {
        ...
        if (del.bondedAmount > 0) {
            ...
            del.bondedAmount = del.bondedAmount - penalty;
    ...
    function claimSnapshotEarnings(
        ...
        emit EarningsClaimed(
            del.delegateAddress,
            msg.sender,
            _pendingStake - del.bondedAmount,
            _pendingFees - del.fees,
            lastClaimRound + 1,
            lip52Round
        );
    ...
    function bondWithHint(
        ...
        if (delegatorStatus(msg.sender) == DelegatorStatus.Unbonded) {
            ...
            delegationAmount = delegationAmount + del.bondedAmount;
        ...
        del.bondedAmount = del.bondedAmount + _amount;
     ...
     */
    function transcoderTotalStake(address _transcoder) public view override returns (uint256) {
     ...
     * @return amount amount of stake locked up by unbonding lock
     ...
     */
    function isActiveTranscoder(address _transcoder) public view override returns (bool) {
        Transcoder storage t = transcoders[_transcoder];
        uint256 currentRound = roundsManager().currentRound();
        ...
        returns (EarningsPool.Data memory pool)
    {
        ...
        if (roundsManager().currentRound() >= lip71Round) {
            if (_round < lip71Round) {
                pool.cumulativeRewardFactor = pool.cumulativeRewardFactor * RESCALE_FACTOR;
                pool.cumulativeFeeFactor = pool.cumulativeFeeFactor * RESCALE_FACTOR;
            } else {
    ...
    function delegatorCumulativeStakeAndFees(
        ...
        for (
            ...
            uint256 lookbackRound = _startRound - lookback;
     ...
     * @return stake ,fees (stake, fees) where stake is the delegator's pending stake and fees is the delegator's pending fees
     */
        ...
        returns (uint256 stake, uint256 fees)
    {
        ...
        while (startRound <= _endRound && startRound <= lip36Round) {
            ...
            if (earningsPool.hasClaimableShares()) {
                ...
                fees = fees + earningsPool.feePoolShare(stake, isTranscoder);
                ...
                stake = stake + earningsPool.rewardPoolShare(stake, isTranscoder);
            }
        ...
        if (_endRound >= lip36Round) {
            ...
            if (isTranscoder) {
                stake = stake + t.cumulativeRewards;
                fees = fees + t.cumulativeFees;
            }
        }
    ...
    function increaseTotalStake(
        ...
        delegators[_delegate].delegatedAmount = delegators[_delegate].delegatedAmount + _amount;
    }
    ...
    function resignTranscoder(address _transcoder) internal {
        ...
        nextRoundTotalActiveStake = nextRoundTotalActiveStake - transcoderTotalStake(_transcoder);
    ...
    function updateDelegatorWithEarnings(
        ...
        if (del.delegateAddress != address(0)) {
            if (startRound <= lip36Round) {
                ...
                require(endLoopRound - _lastClaimRound <= maxEarningsClaimsRounds, "too many rounds to claim through");
            }
    ...
    function processRebond(
        ...
        del.bondedAmount = del.bondedAmount + amount;
