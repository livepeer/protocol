...
contract("LivepeerProtocolIntegration", accounts => {
    ...
    describe("reward flow", () => {
        ...
        it("transcoder should register and delegators should bond to it", async () => {
            ...
            await token.approve(bondingManager.address, 100, {from: accounts[2]})
            await bondingManager.bond(100, accounts[0], {from: accounts[2]})
            // Account 3 bonds to Account 0
            await token.approve(bondingManager.address, 100, {from: accounts[3]})
            await bondingManager.bond(100, accounts[0], {from: accounts[3]})
        })

        it("transcoder should call reward when it is its turn", async () => {
    ...
    describe("job-claim-verify loop", () => {
        ...
        it("transcoder should claim work and verify", async () => {
            const streamId = "1"
