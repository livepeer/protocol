                ...
                describe("self bonding", () => {
                    describe("new transcoder bonds enough to join at last position", () => {
                        ...
                        it("insert new transcoder into the last position and evict the last transcoder (with hint)", async () => {
                            await bondingManager.connect(newTranscoder).bondWithHint(2, newTranscoder.address, ethers.constants.AddressZero, ethers.constants.AddressZero, transcoders[2].address, ethers.constants.AddressZero)
                        })
                    })
                    ...
                    describe("new transcoder bonds enough to join at first position", () => {
                        ...
                        it("insert new transcoder into the first position and evict the last transcoder (with hint)", async () => {
                            await bondingManager.connect(newTranscoder).bondWithHint(size + 1, newTranscoder.address, ethers.constants.AddressZero, ethers.constants.AddressZero, ethers.constants.AddressZero, transcoders[size - 2].address)
                        })
                    })
                })
                ...
                describe("delegation", () => {
                    describe("delegator moving stake can drop first transcoder to last position", () => {
                        ...
                        it("move first transcoder to last position and last transcoder to first position (with hint)", async () => {
                            await bondingManager.connect(delegator).bondWithHint(0, transcoders[1].address, transcoders[1].address, ethers.constants.AddressZero, ethers.constants.AddressZero, transcoders[size - 2].address)
                        })
                    })
                    ...
                    describe("delegator delegates to first transcoder", () => {
                        ...
                        it("delegate to first transcoder (with hint)", async () => {
                            await bondingManager.connect(delegator).bondWithHint(100, transcoders[size - 1].address, ethers.constants.AddressZero, ethers.constants.AddressZero, ethers.constants.AddressZero, transcoders[size - 2].address)
                        })
                    })
                })
            })
            ...
            describe("rebondFromUnbonded", () => {
                ...
                describe("last transcoder is unbonded", () => {
                    ...
                    it("inserts a transcoder back into the last spot (with hint)", async () => {
                        await bondingManager.connect(transcoders[0]).rebondFromUnbondedWithHint(transcoders[0].address, unbondingLockID, transcoders[1].address, ethers.constants.AddressZero)
                    })
                })
                ...
                describe("first transcoder is unbonded", () => {
                    ...
                    it("inserts a transcoder back into the first spot (with hint)", async () => {
                        await bondingManager.connect(transcoders[size - 1]).rebondFromUnbondedWithHint(signers[size - 1].address, unbondingLockID, ethers.constants.AddressZero, transcoders[size - 2].address)
                    })
                })
            })
