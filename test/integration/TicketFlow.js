...
describe("TicketFlow", () => {
    ...
    it("broadcaster funds deposit and penalty escrow", async () => {
        ...
        expect(await ethers.provider.getBalance(minter.address)).to.equal(deposit)

        const reserve = ethers.utils.parseEther("1")
        await broker.connect(broadcaster).fundReserve({value: reserve})
        expect(await ethers.provider.getBalance(minter.address)).to.equal(deposit.add(reserve))
    })
    ...
    it("broadcaster sends a winning ticket and transcoder redeems it", async () => {
        ...
        const deposit = (await broker.getSenderInfo(broadcaster.address)).sender.deposit
        ...
        const endDeposit = (await broker.getSenderInfo(broadcaster.address)).sender.deposit
