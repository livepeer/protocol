...
describe("Delegation", () => {
    ...
    it("next total stake starts off at 0", async () => {
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), 0, "wrong next total stake")
    })
    ...
    it("registers transcoder 1 that self bonds", async () => {
        ...
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), 1000, "wrong next total stake")
    })
    ...
    it("registers transcoder 2 that self bonds", async () => {
        ...
        assert.isTrue(await bondingManager.isRegisteredTranscoder(transcoder2.address), "wrong transcoder status")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), 2000, "wrong next total stake")
    })
    ...
    it("delegator 1 bonds to transcoder 1", async () => {
        ...
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), 3000, "wrong next total stake")
    ...
    it("delegator 2 bonds to transcoder 1", async () => {
        ...
        // Stake counted (+1000) since transcoder is in pool
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), 4000, "wrong next total stake")
    ...
    it("delegator 1 delegates to transcoder 2", async () => {
        ...
        const delegate = (await bondingManager.getDelegator(delegator1.address))[2]
        assert.equal(delegate, transcoder2.address, "delegator 1 delegate incorrect")
        const delegatedStake = (await bondingManager.getDelegator(transcoder2.address))[3]
    ...
    it("delegator 2 delegates to transcoder 2", async () => {
        ...
        const delegate = (await bondingManager.getDelegator(delegator2.address))[2]
    ...
    it("delegator 1 delegates more to transcoder 2", async () => {
        const startBond = (await bondingManager.getDelegator(delegator1.address))[0]

        await token.connect(delegator1).approve(bondingManager.address, 1000)
        await bondingManager.connect(delegator1).bond(1000, transcoder2.address)

        // Stake counted (+1000) since transcoder is in pool
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), 5000, "wrong next total stake")

        const endBond = (await bondingManager.getDelegator(delegator1.address))[0]
    ...
    it("transcoder 1 tries to bond to transcoder 2 and fails", async () => {
        await expect(bondingManager.connect(transcoder1).bond(0, transcoder2.address)).to.be.reverted
    })
    ...
    it("delegator 1 partially unbonds twice (at different times), rebonds with one unbonding lock and withdraws with the other", async () => {
        ...
        const startDelegatorBondedAmount = (await bondingManager.getDelegator(delegator1.address))[0].toNumber()
        ...
        const startTranscoderDelegatedAmount = (await bondingManager.getDelegator(transcoder2.address))[3].toNumber()
        ...
        let lock0 = await bondingManager.getDelegatorUnbondingLock(delegator1.address, unbondingLockID0)
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder2.address), startTranscoderDelegatedAmount - 500, "wrong transcoder delegated stake after unbond 0")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - 500, "wrong next total stake after unbond 0")
        ...
        let lock1 = await bondingManager.getDelegatorUnbondingLock(delegator1.address, unbondingLockID1)
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder2.address), startTranscoderDelegatedAmount - 1200, "wrong transcoder delegated stake after unbond 1")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - 1200, "wrong next total stake after unbond 1")
        ...
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - 700, "wrong next total stake after rebond")
        assert.equal(await bondingManager.transcoderTotalStake(transcoder2.address), startTranscoderDelegatedAmount - 700, "wrong transcoder delegated stake after rebond")

        lock0 = await bondingManager.getDelegatorUnbondingLock(delegator1.address, unbondingLockID0)
        ...
        lock1 = await bondingManager.getDelegatorUnbondingLock(delegator1.address, unbondingLockID1)
    ...
    it("delegator 2 fully unbonds, bonds to transcoder 1 and also rebonds with existing unbonding lock", async () => {
        ...
        const unbondingPeriod = await bondingManager.unbondingPeriod()
        const startDelegatorBondedAmount = (await bondingManager.getDelegator(delegator2.address))[0].toNumber()
        const startDelegatorTokenBalance = await token.balanceOf(delegator2.address)
        const startTranscoder1DelegatedAmount = (await bondingManager.getDelegator(transcoder1.address))[3].toNumber()
        const startTranscoder2DelegatedAmount = (await bondingManager.getDelegator(transcoder2.address))[3].toNumber()
        const startNextTotalStake = (await bondingManager.nextRoundTotalActiveStake()).toNumber()
        ...
        // unbondingLockID = 0
        await bondingManager.connect(delegator2).unbond(startDelegatorBondedAmount)

        // Test state after unbond
        let lock = await bondingManager.getDelegatorUnbondingLock(delegator2.address, unbondingLockID)
        ...
        assert.equal(await bondingManager.delegatorStatus(delegator2.address), DelegatorStatus.Unbonded, "wrong delegator status after unbond")
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder2.address), startTranscoder2DelegatedAmount - startDelegatorBondedAmount, "wrong transcoder delegated stake after unbond")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - startDelegatorBondedAmount, "wrong next total stake after unbond")

        // Delegator 2 tries to rebond and fails because it is no longer bonded
        await expect(bondingManager.connect(delegator2).rebond(unbondingLockID)).to.be.reverted
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder1.address), startTranscoder1DelegatedAmount + 500, "wrong transcoder delegated stake after bond")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - startDelegatorBondedAmount + 500, "wrong next total stake after bond")
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder1.address), startTranscoder1DelegatedAmount + startDelegatorBondedAmount + 500, "wrong transcoder delegated stake after rebond")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake + 500, "wrong next total stake after rebond")

        lock = await bondingManager.getDelegatorUnbondingLock(delegator2.address, unbondingLockID)
    ...
    it("delegator 2 fully unbonds, rebonds from unbonded to transcoder 2 and then rebonds (while not unbonded) with existing unbonding locks", async () => {
        ...
        // Test state after unbond
        let lock = await bondingManager.getDelegatorUnbondingLock(delegator2.address, unbondingLockID0)
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder1.address), startTranscoder1DelegatedAmount - lockAmount0, "wrong transcoder delegated stake after unbond")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - lockAmount0, "wrong next total stake after unbond")
        ...
        // Test state after unbond
        lock = await bondingManager.getDelegatorUnbondingLock(delegator2.address, unbondingLockID1)
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder1.address), startTranscoder1DelegatedAmount - startDelegatorBondedAmount, "wrong transcoder delegated stake after unbond")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - startDelegatorBondedAmount, "wrong next total stake after unbond")

        // Delegator 2 tries to call rebond() and fails because it is no longer bonded
        await expect(bondingManager.connect(delegator2).rebond(unbondingLockID0)).to.be.reverted

        // Delegator 2 rebonds from unbonded to transcoder 2
        await bondingManager.connect(delegator2).rebondFromUnbonded(transcoder2.address, unbondingLockID0)
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder2.address), startTranscoder2DelegatedAmount + lockAmount0, "wrong transcoder delegated stake after rebond from unbonded")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake - startDelegatorBondedAmount + lockAmount0, "wrong next total stake after rebond from unbonded")

        lock = await bondingManager.getDelegatorUnbondingLock(delegator2.address, unbondingLockID0)
        assert.equal(lock[0], 0, "wrong amount for unbonding lock - should be 0")
        assert.equal(lock[1], 0, "wrong withdrawRound for unbonding lock - should be 0")
        ...
        assert.equal(await bondingManager.transcoderTotalStake(transcoder2.address), startTranscoder2DelegatedAmount + startDelegatorBondedAmount, "wrong transcoder delegated stake after rebond")
        assert.equal((await bondingManager.nextRoundTotalActiveStake()).toString(), startNextTotalStake, "wrong next total stake after rebond")
    ...
    it("delegator 1 partially unbonds, earns rewards for 1 round, and rebonds using an unbonding lock", async () => {
        ...
        const endRewardFactor = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder2.address, rewardRound)).cumulativeRewardFactor
        const bondedAmount = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder2.address, rewardRound)).totalStake
        ...
        const startDelegator1BondedAmount = (await bondingManager.getDelegator(delegator1.address))[0]
        const startTranscoder2DelegatedAmount = (await bondingManager.getDelegator(transcoder2.address))[3]
        ...
        assert.equal((await bondingManager.transcoderTotalStake(transcoder2.address)).toString(), startTranscoder2DelegatedAmount.add(BigNumber.from(500)).toString(), "wrong transcoder delegated stake after rebond")
        ...
        const lock = await bondingManager.getDelegatorUnbondingLock(delegator1.address, unbondingLockID)
        assert.equal(lock[0], 0, "wrong amount for unbonding lock - should be 0")
        assert.equal(lock[1], 0, "wrong withdrawRound for unbonding lock - should be 0")
    })
})
