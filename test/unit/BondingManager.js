...
describe("BondingManager", () => {
    ...
    describe("setController", () => {
        ...
        it("should set new Controller", async () => {
            await fixture.controller.updateController(contractId("BondingManager"), signers[0].address)

            assert.equal(await bondingManager.controller(), signers[0].address, "should set new Controller")
        })
    })
    ...
    describe("transcoder", () => {
        ...
        describe("transcoder is not already registered", () => {
            ...
            it("should set transcoder's pending rewardCut and feeShare", async () => {
                ...
                const tInfo = await bondingManager.getTranscoder(signers[0].address)
            ...
            describe("transcoder pool is not full", () => {
                it("should add new transcoder to the pool", async () => {
                    ...
                    assert.equal(
                        await bondingManager.transcoderTotalStake(signers[0].address),
                        1000,
                ...
                it("should add multiple additional transcoders to the pool", async () => {
                    ...
                    await bondingManager.transcoder(5, 10)
                    await bondingManager.connect(signers[1]).bond(1000, signers[1].address)
                    ...
                    assert.equal(
                        await bondingManager.getNextTranscoderInPool(signers[0].address),
                        signers[1].address,
                    ...
                    assert.equal(
                        await bondingManager.transcoderTotalStake(signers[1].address),
                        1000,
            ...
            describe("transcoder pool is full", () => {
                describe("caller has sufficient delegated stake to join pool", () => {
                    it("should evict the transcoder with the least delegated stake and add new transcoder to the pool", async () => {
                        ...
                        assert.isFalse(
                            await bondingManager.isActiveTranscoder(signers[0].address),
        ...
        describe("transcoder is already registered", () => {
            it("should update transcoder's pending rewardCut and feeShare", async () => {
                ...
                let tInfo = await bondingManager.getTranscoder(signers[0].address)
                assert.equal(tInfo[1], 5, "wrong rewardCut")
                assert.equal(tInfo[2], 10, "wrong feeShare")
        ...
        describe("transcoder is active", () => {
            ...
            it("sets rewardCut and feeShare if transcoder has already called reward in the current round", async () => {
                ...
                const transcoder = await bondingManager.getTranscoder(signers[0].address)
    ...
    describe("bond", () => {
        ...
        describe("update transcoder pool", () => {
            ...
            it("should update current earningsPool totalStake when lastActiveStakeUpdateRound < currentRound", async () => {
                const lastActiveStakeUpdateRound = (await bondingManager.getTranscoder(transcoder0.address))
            ...
            it("should not update current earningsPool totalStake when lastActiveStakeUpdateRound = currentRound", async () => {
                ...
                const startActiveStake = (
                    await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 2)
                ).totalStake
                await bondingManager.connect(delegator).bond(1000, transcoder0.address)
            ...
            it("should not update current earningsPool totalStake when lastActiveStakeUpdateRound > currentRound", async () => {
                ...
                const startActiveStake = (
                    await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 1)
                ).totalStake
                await bondingManager.connect(delegator).bond(500, transcoder0.address)
                const endActiveStake = (
                    await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 1)
    ...
    describe("unbond", () => {
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(delegator).unbond(500)).to.be.revertedWith(
                "current round is not initialized"
            )
        })
        ...
        it("should fail if amount is 0", async () => {
            await expect(bondingManager.connect(delegator).unbond(0)).to.be.revertedWith(
                "unbond amount must be greater than 0"
            )
        })

        it("should fail if amount is greater than bonded amount", async () => {
            await expect(bondingManager.connect(delegator).unbond(1001)).to.be.revertedWith(
        ...
        describe("partial unbonding", () => {
            ...
            describe("not delegated to self and delegate is registered transcoder", () => {
                it("should decrease delegated transcoder's delegated stake in pool", async () => {
                    ...
                    assert.equal(
                        await bondingManager.transcoderTotalStake(transcoder.address),
                        1500,
                        "wrong transcoder total stake"
                    )
                })
                ...
                it("should update delegate's lastActiveStakeUpdateRound", async () => {
                    await bondingManager.connect(delegator).unbond(500)
                    assert.equal(
                        (await bondingManager.getTranscoder(transcoder.address)).lastActiveStakeUpdateRound,
                        currentRound + 2
                    )
                })
            })
    ...
    describe("rebond", () => {
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(delegator).rebond(unbondingLockID)).to.be.revertedWith(
                "current round is not initialized"
            )
        })
        ...
        it("should fail for invalid unbonding lock ID", async () => {
            ...
            await expect(bondingManager.connect(delegator).rebond(unbondingLockID + 5)).to.be.revertedWith(
    ...
    describe("rebondFromUnbonded", () => {
        ...
        it("should fail for invalid unbonding lock ID", async () => {
            ...
            // Unbonding lock for ID does not exist
            await expect(
                bondingManager.connect(delegator).rebondFromUnbonded(transcoder.address, unbondingLockID + 5)
        ...
        describe("caller is transferring bond to another address", () => {
            ...
            describe("receiver is not bonded", () => {
                it("should transfer the bond to receiver", async () => {
                    const d1BondedAmountStart = (await bondingManager.getDelegator(delegator1.address)).bondedAmount
                    const d2BondedAmountStart = (await bondingManager.getDelegator(delegator2.address)).bondedAmount

                    expect(d1BondedAmountStart).to.equal(2000)
                    expect(d2BondedAmountStart).to.equal(0)

                    await bondingManager.connect(delegator1).transferBond(
                        delegator2.address,
                        1800,
                        ZERO_ADDRESS,
                        ZERO_ADDRESS,
                    ...
                    const d1DelegateAddress = (await bondingManager.getDelegator(delegator1.address)).delegateAddress
                    const d2DelegateAddress = (await bondingManager.getDelegator(delegator2.address)).delegateAddress

                    expect(d1BondedAmount).to.equal(200, "wrong bonded amount for delegator 1")
                    expect(d2BondedAmount).to.equal(1800, "wrong bonded amount for delegator 2")

                    expect(d1DelegateAddress).to.equal(transcoder0.address, "wrong delegate for delegator 1")
                    expect(d2DelegateAddress).to.equal(transcoder0.address, "wrong delegate for delegator 2")
                })
            })
            ...
            describe("receiver is bonded to the same transcoder", () => {
                ...
                it("should transfer the bond to receiver", async () => {
                    ...
                    const delegatedAmountStart = (await bondingManager.getDelegator(transcoder0.address)).delegatedAmount
                    ...
                    const delegatedAmountEnd = (await bondingManager.getDelegator(transcoder0.address)).delegatedAmount
                    ...
                    expect(d1BondedAmount).to.equal(200, "wrong bonded amount for delegator 1")
                    expect(d2BondedAmount).to.equal(3800, "wrong bonded amount for delegator 2")
            ...
            describe("receiver is bonded to a different transcoder", () => {
                ...
                it("should transfer the bond to receiver", async () => {
                    ...
                    const t0DelegatedAmountStart = (await bondingManager.getDelegator(transcoder0.address)).delegatedAmount
                    const t1DelegatedAmountStart = (await bondingManager.getDelegator(transcoder1.address)).delegatedAmount
                    ...
                    expect(d1DelegateAddress).to.equal(transcoder0.address, "wrong delegate for delegator 1")
                    expect(d2DelegateAddress).to.equal(transcoder1.address, "wrong delegate for delegator 2")

                    const t0DelegatedAmountEnd = (await bondingManager.getDelegator(transcoder0.address)).delegatedAmount
                    const t1DelegatedAmountEnd = (await bondingManager.getDelegator(transcoder1.address)).delegatedAmount

                    expect(t0DelegatedAmountEnd.sub(t0DelegatedAmountStart)).to.equal(-1800, "incorrect t0 delegatedAmount")
                    expect(t1DelegatedAmountEnd.sub(t1DelegatedAmountStart)).to.equal(1800, "incorrect t1 delegatedAmount")
                })
            })
        })
    })
    ...
    describe("withdrawFees", () => {
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(transcoder0).withdrawFees()).to.be.revertedWith(
                "current round is not initialized"
            )
        })
        ...
        it("should withdraw caller's fees", async () => {
            ...
            assert.isAbove(
                (await bondingManager.getDelegator(transcoder0.address))[1].toNumber(),
                0,
    ...
    describe("reward", () => {
        ...
        it("should fail if caller is not an active transcoder for the current round", async () => {
            await expect(bondingManager.connect(nonTranscoder).reward()).to.be.revertedWith(
    ...
    describe("updateTranscoderWithFees", () => {
        ...
        it("should fail if caller is not TicketBroker", async () => {
            await expect(
                bondingManager.updateTranscoderWithFees(transcoder.address, 1000, currentRound + 1)
    ...
    describe("claimEarnings", () => {
        ...
        it("does not update transcoder cumulativeFeeFactor for _endRound EarningsPool if lastFeeRound is in the future", async () => {
            ...
            assert.notEqual(
                (
                    await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, lastFeeRound)
                ).cumulativeFeeFactor.toString(),
                "0"
            )
            assert.equal(
        ...
        it("fires an EarningsClaimed event", async () => {
            ...
            const filter = await bondingManager.filters.EarningsClaimed(transcoder.address, delegator1.address)
            const events = await bondingManager.queryFilter(filter, fromBlock, "latest")
        ...
        describe("caller has a delegate", () => {
            ...
            describe("caller is a transcoder", () => {
                ...
                it("should claim earnings and empty remaining earnings in pool as both a delegator and a transcoder", async () => {
                    ...
                    const earningsPool = await bondingManager.getTranscoderEarningsPoolForRound(
                        transcoder.address,
                        currentRound + 1
                    )
                    assert.equal(
    ...
    describe("pendingStake", () => {
        ...
        describe("delegator is a transcoder", () => {
            it("should return pending rewards as both a delegator and a transcoder", async () => {
                const cumulativeRewards = (await bondingManager.getTranscoder(transcoder.address)).cumulativeRewards.toNumber()
    ...
    describe("isRegisteredTranscoder", () => {
        ...
        describe("address is not registered transcoder", () => {
            it("should return false", async () => {
                assert.isNotOk(
                    await bondingManager.isRegisteredTranscoder(signers[2].address),
