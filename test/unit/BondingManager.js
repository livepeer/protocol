...
describe("BondingManager", () => {
    ...
    describe("setController", () => {
        it("should fail if caller is not Controller", async () => {
            await expect(bondingManager.setController(accounts[0].address)).to.be.revertedWith("caller must be Controller");
        });

        it("should set new Controller", async () => {
            await fixture.controller.updateController(contractId("BondingManager"), accounts[0].address);

            assert.equal(await bondingManager.controller(), accounts[0].address, "should set new Controller");
        });
    });

    describe("setUnbondingPeriod", () => {
        it("should fail if caller is not Controller owner", async () => {
            await expect(bondingManager.connect(accounts[2]).setUnbondingPeriod(5)).to.be.revertedWith("caller must be Controller owner");
        });

        it("should set unbondingPeriod", async () => {
            await bondingManager.setUnbondingPeriod(5);

            assert.equal(await bondingManager.unbondingPeriod(), 5, "wrong unbondingPeriod");
        });
    });

    describe("setNumActiveTranscoders", () => {
        it("should fail if caller is not Controller owner", async () => {
            await expect(bondingManager.connect(accounts[2]).setNumActiveTranscoders(7)).to.be.revertedWith(
    ...
    describe("setMaxEarningsClaimsRounds", () => {
        it("should fail if caller is not Controller owner", async () => {
            await expect(bondingManager.connect(accounts[2]).setMaxEarningsClaimsRounds(2)).to.be.revertedWith(
                "caller must be Controller owner"
            );
        });
    ...
    describe("transcoder", () => {
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.transcoder(5, 10)).to.be.revertedWith("current round is not initialized");
        });
        ...
        it("should fail if the current round is locked", async () => {
            ...
            await expect(bondingManager.transcoder(5, 10)).to.be.revertedWith("can't update transcoder params, current round is locked");
        });

        it("should fail if rewardCut is not a valid percentage <= 100%", async () => {
            await expect(bondingManager.transcoder(PERC_DIVISOR + 1, 10)).to.be.revertedWith("invalid rewardCut percentage");
        });

        it("should fail if feeShare is not a valid percentage <= 100%", async () => {
            await expect(bondingManager.transcoder(5, PERC_DIVISOR + 1)).to.be.revertedWith("invalid feeShare percentage");
        });
        ...
        describe("transcoder is not already registered", () => {
            it("should fail if caller is not delegated to self with a non-zero bonded amount", async () => {
                await expect(bondingManager.transcoder(5, 10)).to.be.revertedWith("transcoder must be registered");
            });
            ...
            it("should set transcoder's pending rewardCut and feeShare", async () => {
                ...
                let tInfo = await bondingManager.getTranscoder(accounts[0].address);
            ...
            describe("transcoder pool is not full", () => {
                it("should add new transcoder to the pool", async () => {
                    ...
                    expect(txRes).to.emit(bondingManager, "TranscoderUpdate").withArgs(accounts[0].address, 5, 10);
                    ...
                    assert.equal(await bondingManager.getFirstTranscoderInPool(), accounts[0].address, "wrong first transcoder in pool");
                    assert.equal(await bondingManager.transcoderTotalStake(accounts[0].address), 1000, "wrong transcoder total stake");
                });

                it("should add multiple additional transcoders to the pool", async () => {
                    await bondingManager.bond(2000, accounts[0].address);
                    await bondingManager.transcoder(5, 10);
                    await bondingManager.connect(accounts[1]).bond(1000, accounts[1].address);
                    ...
                    assert.equal(await bondingManager.getFirstTranscoderInPool(), accounts[0].address, "wrong first transcoder in pool");
                    assert.equal(
                        await bondingManager.getNextTranscoderInPool(accounts[0].address),
                        accounts[1].address,
                        "wrong second transcoder in pool"
                    );
                    assert.equal(
                        await bondingManager.transcoderTotalStake(accounts[0].address),
                        2000,
                        "wrong first transcoder total stake"
                    );
                    assert.equal(
                        await bondingManager.transcoderTotalStake(accounts[1].address),
                        1000,
            ...
            describe("transcoder pool is full", () => {
                describe("caller has sufficient delegated stake to join pool", () => {
                    it("should evict the transcoder with the least delegated stake and add new transcoder to the pool", async () => {
                        ...
                        await Promise.all(
                            transcoders.map((account, idx) => {
                                return bondingManager
                                    ...
                                    .then(() => {
                                        return bondingManager.connect(account).transcoder(5, 10);
                                    });
                            })
                        );
                        ...
                        assert.equal(await bondingManager.getTranscoderPoolSize(), 2, "wrong transcoder pool size");
                        assert.equal(
                            await bondingManager.transcoderTotalStake(newTranscoder.address),
                            6000,
                            "wrong transcoder total stake"
                        );
                        assert.isFalse(
                            await bondingManager.isActiveTranscoder(accounts[0].address),
        ...
        describe("transcoder is active", () => {
            ...
            it("fails if transcoder has not called reward for the current round", async () => {
                await expect(bondingManager.transcoder(10, 20)).to.be.revertedWith(
            ...
            it("sets rewardCut and feeShare if transcoder has already called reward in the current round", async () => {
                ...
                const transcoder = await bondingManager.getTranscoder(accounts[0].address);
    ...
    describe("bond", () => {
        ...
        beforeEach(async () => {
            ...
            transcoder2 = accounts[2];
            nonTranscoder = accounts[9];
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(delegator).bond(1000, transcoder0.address)).to.be.revertedWith(
        ...
        describe("update transcoder pool", () => {
            beforeEach(async () => {
                await bondingManager.connect(transcoder2).bond(500, transcoder2.address);
            ...
            it("adds a new transcoder to the pool", async () => {
                await bondingManager.connect(transcoder2).bond(1000, transcoder2.address);
            ...
            it("should update current earningsPool totalStake when lastActiveStakeUpdateRound < currentRound", async () => {
                const lastActiveStakeUpdateRound = (await bondingManager.getTranscoder(transcoder0.address)).lastActiveStakeUpdateRound;
                ...
                const lastActiveStake = (
                    await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, lastActiveStakeUpdateRound)
                ).totalStake;
                const pool = await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 1);
            ...
            it("should not update current earningsPool totalStake when lastActiveStakeUpdateRound = currentRound", async () => {
                await bondingManager.connect(delegator).bond(500, transcoder0.address);
                ...
                const startActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 2))
                    .totalStake;
                await bondingManager.connect(delegator).bond(1000, transcoder0.address);
                const endActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 2))
                    .totalStake;
            ...
            it("should not update current earningsPool totalStake when lastActiveStakeUpdateRound > currentRound", async () => {
                ...
                const startActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 1))
                    .totalStake;
                await bondingManager.connect(delegator).bond(500, transcoder0.address);
                const endActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder0.address, currentRound + 1))
                    .totalStake;
            ...
            describe("evicts a transcoder from the pool", () => {
                it("last transcoder gets evicted and new transcoder gets inserted", async () => {
                    ...
                    await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 2);
                    assert.isTrue(await bondingManager.isActiveTranscoder(transcoder2.address));
                    assert.isTrue(await bondingManager.isActiveTranscoder(transcoder1.address));
                    assert.isFalse(await bondingManager.isActiveTranscoder(transcoder0.address));
                    assert.equal((await bondingManager.transcoderTotalStake(transcoder2.address)).toString(), "2500");
                });

                it("sets deactivationRound for the inserted transcoder to the max possible round number", async () => {
                    await bondingManager.connect(delegator).bond(2000, transcoder2.address);
                    assert.equal((await bondingManager.getTranscoder(transcoder2.address)).deactivationRound, 2 ** 256 - 1);
                });

                it("sets the deactivationRound for the evicted transcoder to the next round", async () => {
                    await bondingManager.connect(delegator).bond(2000, transcoder2.address);
                    assert.equal((await bondingManager.getTranscoder(transcoder0.address)).deactivationRound, currentRound + 2);
                });
            ...
            it("inserts into pool without evicting if pool is not full", async () => {
                ...
                await bondingManager.connect(delegator).bond(2500, transcoder2.address);
                await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 2);
                assert.isTrue(await bondingManager.isActiveTranscoder(transcoder2.address));
                assert.isTrue(await bondingManager.isActiveTranscoder(transcoder0.address));
                // transcoder 2 should be first
                assert.equal(await bondingManager.getFirstTranscoderInPool(), transcoder2.address);
            });

            it("doesn't insert into pool when stake is too low", async () => {
                await bondingManager.connect(delegator).bond(10, transcoder2.address);
                await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 2);
                assert.isFalse(await bondingManager.isActiveTranscoder(transcoder2.address));
            });

            it("updates total stake in earnings pool for next round", async () => {
                await bondingManager.connect(delegator).bond(2000, transcoder2.address);
                const poolT2 = await bondingManager.getTranscoderEarningsPoolForRound(transcoder2.address, currentRound + 2);
        ...
        describe("caller is unbonded", () => {
            it("should fail if provided amount = 0", async () => {
                await expect(bondingManager.connect(delegator).bond(0, transcoder0.address)).to.be.revertedWith(
            ...
            it("should set delegate", async () => {
                ...
                assert.equal((await bondingManager.getDelegator(delegator.address))[2], transcoder0.address, "wrong delegateAddress");
            });
            ...
            describe("delegate is a registered transcoder", () => {
                it("should increase transcoder's delegated stake in pool", async () => {
                    ...
                    const startTranscoderTotalStake = await bondingManager.transcoderTotalStake(transcoder0.address);
                    await bondingManager.connect(delegator).bond(1000, transcoder0.address);
                    const endNextTotalStake = await bondingManager.nextRoundTotalActiveStake();
                    const endTranscoderTotalStake = await bondingManager.transcoderTotalStake(transcoder0.address);
                ...
                it("should update delegate's position in transcoder pool", async () => {
                    await bondingManager.connect(delegator).bond(3000, transcoder0.address);
                    assert.equal(
                ...
                it("should update transcoder's lastActiveStakeUpdateRound", async () => {
                    await bondingManager.connect(delegator).bond(3000, transcoder0.address);
                    assert.equal((await bondingManager.getTranscoder(transcoder0.address)).lastActiveStakeUpdateRound, currentRound + 1);
                });
            });
            describe("delegate is not a registered transcoder", () => {
                it("should not update next total stake", async () => {
                    const startNextTotalStake = await bondingManager.nextRoundTotalActiveStake();
                    await bondingManager.connect(delegator).bond(1000, nonTranscoder.address);
                    const endNextTotalStake = await bondingManager.nextRoundTotalActiveStake();
                ...
                it("should not update transcoder's lastActiveStakeUpdateRound", async () => {
                    await bondingManager.connect(delegator).bond(3000, nonTranscoder.address);
                    assert.equal((await bondingManager.getTranscoder(nonTranscoder.address)).lastActiveStakeUpdateRound, 0);
                });
            });
        });
        describe("caller is bonded", () => {
            ...
            describe("caller is changing delegate", () => {
                ...
                it("should set startRound to next round", async () => {
                    await bondingManager.connect(delegator).bond(0, transcoder1.address);
                    assert.equal((await bondingManager.getDelegator(delegator.address))[4], currentRound + 1, "wrong startRound");
                });
                ...
                it("should set new delegate", async () => {
                    await bondingManager.connect(delegator).bond(0, transcoder1.address);
                    assert.equal((await bondingManager.getDelegator(delegator.address))[2], transcoder1.address, "wrong delegateAddress");
                });
                describe("old delegate is registered transcoder", () => {
                    describe("new delegate is a registered transcoder", () => {
                        ...
                        it("should update old delegate and new delegate's lastActiveStakeUpdateRound", async () => {
                            await bondingManager.connect(delegator).bond(0, transcoder1.address);
                            assert.equal(
                                (await bondingManager.getTranscoder(transcoder0.address)).lastActiveStakeUpdateRound,
                                currentRound + 1
                            );
                            assert.equal(
                                (await bondingManager.getTranscoder(transcoder1.address)).lastActiveStakeUpdateRound,
                                currentRound + 1
                            );
                        });
                    });
                    describe("new delegate is not a registered transcoder", () => {
                        it("should not update new delegate's position in transcoder pool", async () => {
                            await bondingManager.connect(delegator).bond(0, nonTranscoder.address);
                            await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 1);
                            assert.isFalse(await bondingManager.isActiveTranscoder(nonTranscoder.address));
                        });
                ...
                describe("old delegate is not a registered transcoder", () => {
                    beforeEach(async () => {
                        await bondingManager.connect(delegator).bond(0, delegator2.address);
                    });
                    describe("new delegate is a registered transcoder", () => {
                        it("should update new delegate's position in transcoder pool", async () => {
                            await bondingManager.connect(delegator).bond(0, transcoder0.address);
                            await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 1);
                            // New delegate was not previously first transcoder in pool and now is
                            assert.equal(
                                await bondingManager.getFirstTranscoderInPool(),
                        ...
                        it("should only update new delegate lastActiveStakeUpdateRound", async () => {
                            ...
                            assert.equal((await bondingManager.getTranscoder(delegator2.address)).lastActiveStakeUpdateRound, 0);
                        });
                    });
                    describe("new delegate is not a registered transcoder()", () => {
                        it("should not update the new delegate's position in transcoder pool", async () => {
                            ...
                            await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 1);
                            assert.isFalse(
                                await bondingManager.isActiveTranscoder(nonTranscoder.address),
                                "did not correctly update position in pool"
                            );
                        });
                ...
                describe("caller is just moving bonded stake because provided amount = 0", () => {
                    it("should update new delegate's delegated amount with current bonded stake", async () => {
                        const startDelegatedAmount = (await bondingManager.getDelegator(transcoder1.address))[3];
                        await bondingManager.connect(delegator).bond(0, transcoder1.address);
                        const endDelegatedAmount = (await bondingManager.getDelegator(transcoder1.address))[3];
                    ...
                    it("should not update bonded amount", async () => {
                        const startBondedAmount = (await bondingManager.getDelegator(delegator.address))[0];
                        await bondingManager.connect(delegator).bond(0, transcoder1.address);
                        const endBondedAmount = (await bondingManager.getDelegator(delegator.address))[0];
                    ...
                    describe("new delegate is registered transcoder", () => {
                        it("should increase transcoder's total stake in pool with current bonded stake", async () => {
                            const startTranscoderTotalStake = await bondingManager.transcoderTotalStake(transcoder1.address);
                            await bondingManager.connect(delegator).bond(0, transcoder1.address);
                            const endTranscoderTotalStake = await bondingManager.transcoderTotalStake(transcoder1.address);
                            assert.equal(
                ...
                describe("caller is increasing and moving bonded stake because provided amount > 0", () => {
                    it("should update new delegate's delegated amount with current bonded stake + provided amount", async () => {
                        const startDelegatedAmount = (await bondingManager.getDelegator(transcoder1.address))[3];
                        await bondingManager.connect(delegator).bond(1000, transcoder1.address);
                        const endDelegatedAmount = (await bondingManager.getDelegator(transcoder1.address))[3];
                    ...
                    describe("new delegate is registered transcoder", () => {
                        ...
                        describe("old delegate is not registered transcoder", () => {
                            it("should increase next total stake by current bonded stake + additional bonded stake", async () => {
                                // Delegate to non-transcoder
                                await bondingManager.connect(delegator).bond(0, nonTranscoder.address);
                                const bondedAmount = (await bondingManager.getDelegator(delegator.address))[0].toNumber();
                                const startNextTotalStake = await bondingManager.nextRoundTotalActiveStake();
                                await bondingManager.connect(delegator).bond(1000, transcoder1.address);
                                const endNextTotalStake = await bondingManager.nextRoundTotalActiveStake();
                                assert.equal(
        ...
        describe("set delegate earnings pool factors if not initialized", () => {
            it("sets cumulativeRewardFactor if value is zero", async () => {
                await bondingManager.connect(delegator).bond(100, transcoder0.address);
                await bondingManager.connect(transcoder1).reward();
                const ep0 = await bondingManager.getTranscoderEarningsPoolForRound(transcoder1.address, currentRound);
                await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 1);
                await bondingManager.connect(delegator).bond(100, transcoder1.address);
    ...
    describe("unbond", () => {
        ...
        beforeEach(async () => {
            ...
            await bondingManager.connect(delegator).bond(1000, transcoder.address);
            await bondingManager.connect(delegator2).bond(1000, delegator.address);
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(delegator).unbond(500)).to.be.revertedWith("current round is not initialized");
        });
        ...
        it("should fail if the caller is not bonded", async () => {
            ...
            await expect(bondingManager.connect(delegator).unbond(500)).to.be.revertedWith("caller must be bonded");
        });

        it("should fail if amount is 0", async () => {
            await expect(bondingManager.connect(delegator).unbond(0)).to.be.revertedWith("unbond amount must be greater than 0");
        });

        it("should fail if amount is greater than bonded amount", async () => {
            await expect(bondingManager.connect(delegator).unbond(1001)).to.be.revertedWith("amount is greater than bonded amount");
        });
        ...
        it("should update current earningsPool totalStake when lastActiveStakeUpdateRound < currentRound", async () => {
            ...
            const lastActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 1))
                .totalStake;
        ...
        it("should not update current earningsPool totalStake when lastActiveStakeUpdateRound = currentRound", async () => {
            assert.equal((await bondingManager.getTranscoder(transcoder.address)).lastActiveStakeUpdateRound.toNumber(), currentRound + 1);

            const startActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 1))
                .totalStake;
            await bondingManager.connect(delegator).unbond(1000);
            const endActiveStake = (await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 1))
                .totalStake;
        ...
        describe("partial unbonding", () => {
            it("should create an unbonding lock for a partial unbond", async () => {
                ...
                const lock = await bondingManager.getDelegatorUnbondingLock(delegator.address, unbondingLockID);
                ...
                const tInfo = await bondingManager.getDelegator(transcoder.address);
            ...
            it("should fire an Unbond event with an unbonding lock representing a partial unbond", async () => {
                ...
                expect(txRes)
                    ...
                    .withArgs(transcoder.address, delegator.address, unbondingLockID.toNumber(), 500, currentRound + 1 + unbondingPeriod);
            });
            ...
            describe("delegated to non-transcoder", () => {
                ...
                it("should not change delegate's lastActiveStakeUpdateRound", async () => {
                    await bondingManager.connect(delegator).unbond(500);
                    assert.equal((await bondingManager.getTranscoder(delegator.address)).lastActiveStakeUpdateRound, 0);
                });
            });
            ...
            describe("not delegated to self and delegate is registered transcoder", () => {
                it("should decrease delegated transcoder's delegated stake in pool", async () => {
                    ...
                    assert.equal(await bondingManager.transcoderTotalStake(transcoder.address), 1500, "wrong transcoder total stake");
                });
                ...
                it("should update delegate's lastActiveStakeUpdateRound", async () => {
                    await bondingManager.connect(delegator).unbond(500);
                    assert.equal((await bondingManager.getTranscoder(transcoder.address)).lastActiveStakeUpdateRound, currentRound + 2);
                });
            });
        ...
        describe("full unbonding", () => {
            ...
            it("should fire an Unbond event with an unbonding lock representing a full unbond", async () => {
                ...
                expect(txRes)
                    .to.emit(bondingManager, "Unbond")
                    .withArgs(transcoder.address, delegator.address, unbondingLockID.toNumber(), 1000, currentRound + 1 + unbondingPeriod);
            });
            ...
            describe("is an active transcoder", () => {
                it("should resign as a transcoder", async () => {
                    ...
                    assert.isFalse(await bondingManager.isRegisteredTranscoder(transcoder.address), "wrong transcoder status");
                });
    ...
    describe("rebond", () => {
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(delegator).rebond(unbondingLockID)).to.be.revertedWith("current round is not initialized");
        });
        ...
        it("should fail if delegator is not in the Bonded or Pending state", async () => {
            ...
            await expect(bondingManager.connect(delegator).rebond(unbondingLockID)).to.be.revertedWith("caller must be bonded");
        });

        it("should fail for invalid unbonding lock ID", async () => {
            // Unbonding lock for ID does not exist
            await expect(bondingManager.connect(delegator).rebond(unbondingLockID + 5)).to.be.revertedWith("invalid unbonding lock ID");
        });
        ...
        describe("current delegate is a registered transcoder", () => {
            it("should increase transcoder's delegated stake in pool", async () => {
                ...
                assert.equal(await bondingManager.transcoderTotalStake(transcoder.address), 2000, "wrong transcoder total stake");
            });
            ...
            it("should evict when rebonding and pool is full", async () => {
                await bondingManager.connect(transcoder1).bond(1900, transcoder1.address);
                await bondingManager.connect(transcoder1).transcoder(5, 10);
                ...
                await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound + 2);
                assert.isTrue(await bondingManager.isActiveTranscoder(transcoder.address));
                // Check that transcoder2's deactivation round is the next round
                assert.equal((await bondingManager.getTranscoder(transcoder2.address)).deactivationRound, currentRound + 2);
                const pool = await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 2);
        ...
        it("should create an Rebond event", async () => {
            const txRes = await bondingManager.connect(delegator).rebond(unbondingLockID);
            expect(txRes).to.emit(bondingManager, "Rebond").withArgs(transcoder.address, delegator.address, unbondingLockID, 500);
        });
    });
    ...
    describe("rebondFromUnbonded", () => {
        ...
        it("should fail if system is paused", async () => {
            ...
            await expect(bondingManager.connect(delegator).rebondFromUnbonded(transcoder.address, unbondingLockID)).to.be.revertedWith(
                "system is paused"
            );
        });
        ...
        it("should fail for invalid unbonding lock ID", async () => {
            ...
            // Unbonding lock for ID does not exist
            await expect(bondingManager.connect(delegator).rebondFromUnbonded(transcoder.address, unbondingLockID + 5)).to.be.revertedWith(
        ...
        it("should set delegator's start round and delegate address", async () => {
            ...
            await bondingManager.connect(delegator).rebondFromUnbonded(transcoder.address, unbondingLockID);
        ...
        describe("new delegate is not a registered transcoder", () => {
            ...
            it("should not change the total active stake for the next round", async () => {
                const startTotalActiveStake = await bondingManager.nextRoundTotalActiveStake();
                await bondingManager.connect(delegator).rebondFromUnbonded(nonTranscoder.address, unbondingLockID);
                const endTotalActiveStake = await bondingManager.nextRoundTotalActiveStake();
                assert.equal(endTotalActiveStake.sub(startTotalActiveStake), 0, "wrong change in nextRoundTotalActiveStake");
            });
    ...
    describe("withdrawStake", () => {
        ...
        it("should fail if system is paused", async () => {
            ...
            await expect(bondingManager.connect(delegator).withdrawStake(unbondingLockID)).to.be.revertedWith("system is paused");
        });
        ...
        it("should fail if unbonding lock is invalid", async () => {
            // Unbonding lock for ID does not exist
            await expect(bondingManager.connect(delegator).withdrawStake(unbondingLockID + 5)).to.be.revertedWith(
                "invalid unbonding lock ID"
            );
        });

        it("should fail if unbonding lock withdraw round is in the future", async () => {
            await expect(bondingManager.connect(delegator).withdrawStake(unbondingLockID)).revertedWith(
        ...
        it("should withdraw tokens for unbonding lock", async () => {
            ...
            await bondingManager.connect(delegator).withdrawStake(unbondingLockID);
    ...
    describe("withdrawFees", () => {
        ...
        it("should fail if current round is not initialized", async () => {
            ...
            await expect(bondingManager.connect(transcoder0).withdrawFees()).to.be.revertedWith("current round is not initialized");
        });

        it("should fail if there are no fees to withdraw", async () => {
            await expect(bondingManager.connect(transcoder1).withdrawFees()).to.be.revertedWith("no fees to withdraw");
        });

        it("should withdraw caller's fees", async () => {
            await bondingManager.connect(transcoder0).claimEarnings(currentRound + 1);
            assert.isAbove((await bondingManager.getDelegator(transcoder0.address))[1].toNumber(), 0, "caller should have non-zero fees");
    ...
    describe("claimEarnings", () => {
        ...
        beforeEach(async () => {
            ...
            await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound);
            await bondingManager.connect(delegator1).bond(3000, transcoder.address);
            await bondingManager.connect(delegator2).bond(3000, transcoder.address);
            await bondingManager.connect(delegator3).bond(3000, transcoder.address);
            await fixture.roundsManager.execute(bondingManager.address, functionSig("setCurrentRoundTotalActiveStake()"));
        ...
        it("should fail if system is paused", async () => {
            ...
            await expect(bondingManager.connect(delegator1).claimEarnings(currentRound + 1)).to.be.revertedWith("system is paused");
        });
        ...
        it("does not update transcoders cumulativeRewardFactor for _endRound EarningsPool if reward is not called for _endRound yet when _endRound < LIP-36 upgrade round", async () => {
            ...
            const lastRewardPool = await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 1);
        ...
        it("updates transcoders cumulativeRewardFactor for _endRound EarningsPool if reward is not called for _endRound yet when _endRound == LIP-36 upgrade round", async () => {
            ...
            assert.equal(
                ...
                (await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 1)).cumulativeRewardFactor
            );
        });
        ...
        it("updates transcoders cumulativeFeeFactor for _endRound EarningsPool if no fees for _endRound yet when _endRound == LIP-36 upgrade round", async () => {
            ...
            assert.equal(
                ...
                (await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, currentRound + 1)).cumulativeFeeFactor
            );
        });
        ...
        it("updates transcoders cumulativeRewardFactor for _endRound EarningsPool with rescaled value when lastRewardRound < LIP-71 round", async () => {
            ...
            assert.ok(
                new BN(lrrPool.cumulativeRewardFactor.toString()).mul(rescaleFactor).eq(new BN(pool.cumulativeRewardFactor.toString()))
            );
        });
        ...
        it("updates transcoders cumulativeFeeFactor for _endRound EarningsPool with rescaled value when lastFeeRound < LIP-71 round", async () => {
            ...
            // that rescaling occurred
            const rescaleFactor = constants.PERC_DIVISOR_PRECISE.div(new BN(constants.PERC_DIVISOR));
            assert.ok(new BN(lrrPool.cumulativeFeeFactor.toString()).mul(rescaleFactor).eq(new BN(pool.cumulativeFeeFactor.toString())));
        });
        ...
        it("does not update transcoder cumulativeRewardFactor for _endRound EarningsPool if lastRewardRound is in the future", async () => {
            ...
            const lastRewardRound = (await bondingManager.getTranscoder(transcoder.address)).lastRewardRound;
            assert.equal(lastRewardRound.toNumber(), currentRound + 3);
            assert.notEqual(
                (
                    await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, lastRewardRound)
                ).cumulativeRewardFactor.toString(),
                "0"
            );
            assert.equal(
        ...
        it("does not update transcoder cumulativeFeeFactor for _endRound EarningsPool if lastFeeRound is in the future", async () => {
            ...
            const lastFeeRound = (await bondingManager.getTranscoder(transcoder.address)).lastFeeRound;
            assert.equal(lastFeeRound.toNumber(), currentRound + 3);
            assert.notEqual(
                (await bondingManager.getTranscoderEarningsPoolForRound(transcoder.address, lastFeeRound)).cumulativeFeeFactor.toString(),
                "0"
            );
            assert.equal(
                (
        ...
        it("fires an EarningsClaimed event", async () => {
            ...
            const txResult = await bondingManager.connect(delegator1).claimEarnings(currentRound + 1);
        ...
        describe("caller has a delegate", () => {
            ...
            it("should claim earnings for 1 round", async () => {
                ...
                await bondingManager.connect(delegator1).claimEarnings(currentRound + 1);
                const endDInfo1 = await bondingManager.getDelegator(delegator1.address);
                const d1Rewards = endDInfo1[0].sub(startDInfo1[0]);
                const d1Fees = endDInfo1[1].sub(startDInfo1[1]);

                const startDInfo2 = await bondingManager.getDelegator(delegator2.address);
                await bondingManager.connect(delegator2).claimEarnings(currentRound + 1);
                const endDInfo2 = await bondingManager.getDelegator(delegator2.address);
                const d2Rewards = endDInfo2[0].sub(startDInfo2[0]);
                const d2Fees = endDInfo2[1].sub(startDInfo2[1]);

                const startDInfo3 = await bondingManager.getDelegator(delegator3.address);
                await bondingManager.connect(delegator3).claimEarnings(currentRound + 1);
                const endDInfo3 = await bondingManager.getDelegator(delegator3.address);
            ...
            it("should claim earnings for > 1 round", async () => {
                ...
                const startDInfo2 = await bondingManager.getDelegator(delegator2.address);
                await bondingManager.connect(delegator2).claimEarnings(currentRound + 2);
                const endDInfo2 = await bondingManager.getDelegator(delegator2.address);
                const d2Rewards = endDInfo2[0].sub(startDInfo2[0]);
                const d2Fees = endDInfo2[1].sub(startDInfo2[1]);

                const startDInfo3 = await bondingManager.getDelegator(delegator3.address);
                await bondingManager.connect(delegator3).claimEarnings(currentRound + 2);
                const endDInfo3 = await bondingManager.getDelegator(delegator3.address);
                const d3Rewards = endDInfo3[0].sub(startDInfo3[0]);
                const d3Fees = endDInfo3[1].sub(startDInfo3[1]);
            ...
            describe("caller is a transcoder", () => {
                it("should claim earnings as both a delegator and a transcoder", async () => {
                    ...
                    await bondingManager.connect(transcoder).claimEarnings(currentRound + 1);
                    const endDInfo = await bondingManager.getDelegator(transcoder.address);
    ...
    describe("claimSnapshotEarnings", () => {
        ...
        it("reverts if current round is not initialized", async () => {
            ...
            await expect(bondingManager.claimSnapshotEarnings(1500, 1000, [], [])).to.be.revertedWith("current round is not initialized");
        });
        ...
        it("reverts if the delegator has already claimed past the LIP-52 upgrade round", async () => {
            ...
            await expect(bondingManager.claimSnapshotEarnings(1500, 1000, [], [])).to.be.revertedWith("Already claimed for LIP-52");
        });
        ...
        it("reverts if proof is invalid", async () => {
            ...
            await expect(bondingManager.claimSnapshotEarnings(1500, 1000, [], [])).to.be.revertedWith("Merkle proof is invalid");
        });
    ...
    describe("pendingStake", () => {
        ...
        it("should return delegator.bondedAmount when endRound < lastClaimRound", async () => {
            await bondingManager.connect(delegator).claimEarnings(currentRound + 1);

            const bondedAmount = (await bondingManager.getDelegator(delegator.address)).bondedAmount;
            assert.equal((await bondingManager.pendingStake(delegator.address, currentRound)).toString(), bondedAmount.toString());
        });
        ...
        it("should return delegator.bondedAmount when endRound = lastClaimRound", async () => {
            ...
            const bondedAmount = (await bondingManager.getDelegator(delegator.address)).bondedAmount;
            assert.equal((await bondingManager.pendingStake(delegator.address, currentRound + 1)).toString(), bondedAmount.toString());
        });
        ...
        it("should return pending rewards even if transcoder hasn't called reward for it's lastClaimRound", async () => {
            ...
            assert.equal(
                (await bondingManager.pendingStake(delegator.address, currentRound + 3)).toNumber(),
        ...
        describe("no rewards since last claim round", async () => {
            ...
            it("should return bondedAmount when transcoder.lastRewardRound < delegator.lastClaimRound", async () => {
                ...
                assert.equal((await bondingManager.pendingStake(delegator.address, currentRound + 3)).toNumber(), bondedAmount);
            });
        });

        describe("delegator is a transcoder", () => {
            it("should return pending rewards as both a delegator and a transcoder", async () => {
                let cumulativeRewards = (await bondingManager.getTranscoder(transcoder.address)).cumulativeRewards.toNumber();
                const pendingRewards = 250 + Math.floor((500 * ((1250 * PERC_DIVISOR) / 3000)) / PERC_DIVISOR) + cumulativeRewards;

                assert.equal(
                    (await bondingManager.pendingStake(transcoder.address, currentRound)).toNumber(),
                    1000 + pendingRewards,
        ...
        describe("cumulative factors rescaling", () => {
            it("should rescale cumulativeRewardFactor based on whether it was stored before or after the LIP-71 round", async () => {
                ...
                const rewards3 = Math.floor((500 * (((1000 + rewards0 + rewards1 + rewards2) * PERC_DIVISOR) / 5000)) / PERC_DIVISOR);
    ...
    describe("pendingFees", () => {
        ...
        it("should return pending fees for all rounds since lastClaimRound when endRound < currentRound and endRound > LIP-36 upgrade round", async () => {
            ...
            const pf = await bondingManager.pendingFees(delegator.address, currentRound + 1);
            assert.equal(pf.toString(), pendingFees0 + pendingFees1, "should return sum of collected fees and pending fees for 2 rounds");
            assert.equal(pf.toString(), (await bondingManager.pendingFees(delegator.address, currentRound + 2)).toString());
        });
        ...
        it("should return pending fees for all rounds since lastClaimRound when endRound == currentRound and endRound > LIP-36 upgrade round", async () => {
            ...
            assert.equal(
                (await bondingManager.pendingFees(delegator.address, currentRound + 2)).toNumber(),
        ...
        it("should return pending fees when transcoder has claimed earnings since LIP36", async () => {
            ...
            assert.equal((await bondingManager.pendingFees(delegator.address, currentRound + 4)).toString(), pendingFees2.toString());
        });
        ...
        it("should return pending fees when transcoder hasn't called reward for the previous round but has for the current round", async () => {
            ...
            assert.equal(
                (await bondingManager.pendingFees(delegator.address, currentRound + 5)).toString(),
        ...
        describe("no fees since lastClaimRound", async () => {
            ...
            it("should return current fees when there are no additional fees since last claim round", async () => {
                ...
                assert.equal((await bondingManager.pendingFees(delegator.address, currentRound + 4)).toNumber(), fees);
            });
        });
        ...
        describe("delegator is a transcoder", () => {
            it("should return pending fees as both a delegator and a transcoder", async () => {
                ...
                assert.equal(
                    (await bondingManager.pendingFees(transcoder.address, currentRound + 1)).toNumber(),
                    pendingFees,
