...
describe("TicketBroker", () => {
    ...
    before(async () => {
        ...
        broker = await (await ethers.getContractFactory("TicketBroker")).deploy(fixture.controller.address)
    ...
    describe("fundDeposit", () => {
        ...
        it("track multiple sender's deposits", async () => {
            ...
            const deposit = (await broker.getSenderInfo(sender)).sender.deposit.toString()
            const deposit2 = (await broker.getSenderInfo(sender2.address)).sender.deposit.toString()
    ...
    describe("fundReserve", () => {
        ...
        it("track multiple sender's reserves", async () => {
            ...
            const reserve = (await broker.getSenderInfo(sender)).reserve.fundsRemaining.toString()
            const reserve2 = (await broker.getSenderInfo(sender2.address)).reserve.fundsRemaining.toString()
        ...
        it("preserves remaining funds when reserve was claimed from", async () => {
            ...
            await fixture.roundsManager.setMockUint256(functionSig("currentRound()"), currentRound)
            await fixture.stakingManager.setMockUint256(functionSig("getTranscoderPoolSize()"), numRecipients)
            await fixture.stakingManager.setMockBool(functionSig("isActiveTranscoder(address)"), true)
    ...
    describe("redeemWinningTicket", () => {
        ...
        it("reverts if ticket auxData != 64 bytes", async () => {
            ...
            ).to.be.revertedWith("invalid length for ticket auxData: must be 64 bytes")
        })
        ...
        it("reverts if block hash for ticket creationRound is null", async () => {
            ...
            ).to.be.revertedWith("ticket creationRound does not have a block hash")
        })
        ...
        it("reverts if ticket creationRoundBlockHash is invalid for ticket creationRound", async () => {
            ...
            ).to.be.revertedWith("ticket creationRoundBlockHash invalid for creationRound")
        })
        ...
        it("reverts if recipientRand is not the preimage for the ticket's recipientRandHash", async () => {
            ...
            ).to.be.revertedWith("recipientRand does not match recipientRandHash")
        })
        ...
        describe("deposit < faceValue", () => {
            describe("sender.deposit is zero", () => {
                it("does not allow a claim if there are no registered recipients", async () => {
                    ...
                    await fixture.stakingManager.setMockUint256(functionSig("getTranscoderPoolSize()"), 0)
                    await fixture.stakingManager.setMockBool(functionSig("isActiveTranscoder(address)"), false)
                    await broker.fundReserve({value: 1000})
                ...
                it("allows a claim from a registered recipient", async () => {
                    ...
                    const filter = await fixture.stakingManager.filters.UpdateTranscoderWithFees()
                ...
                it("allows claims from all registered recipients for their full reserve allocations", async () => {
                    ...
                    await broker.connect(recipient2).redeemWinningTicket(ticket2, senderSig2, recipientRand)
                    ...
                    assert.equal((await broker.claimedReserve(sender, recipient2.address)).toString(), allocation.toString())
                })
            })
        ...
        it("does not transfer sender.deposit to recipient when faceValue is zero", async () => {
            ...
            expect(tx).to.not.emit(fixture.stakingManager, "UpdateTranscoderWithFees")
            const endDeposit = (await broker.getSenderInfo(sender)).sender.deposit.toString()
        ...
        it("can be called by an account that is not the recipient", async () => {
            ...
            await broker.connect(thirdParty).redeemWinningTicket(ticket, senderSig, recipientRand)
        ...
        it("emits a WinningTicketRedeemed event with indexed sender", async () => {
            ...
            const faceValue = 1500
            const ticket = createWinningTicket(recipient, sender, recipientRand, faceValue)
            const ticket2 = createWinningTicket(recipient, sender2.address, recipientRand, faceValue)
            const senderSig = await signMsg(getTicketHash(ticket), sender)
            const senderSig2 = await signMsg(getTicketHash(ticket2), sender2.address)
            const fromBlock = (await web3.eth.getBlock("latest")).number

            await broker.connect(signers[1]).redeemWinningTicket(ticket, senderSig, recipientRand)
            await broker.connect(signers[1]).redeemWinningTicket(ticket2, senderSig2, recipientRand)
        ...
        it("emits a WinningTicketRedeemed event with indexed recipient", async () => {
            ...
            const faceValue = 200
            const ticket = createWinningTicket(recipient, sender, recipientRand, faceValue)
            const ticket2 = createWinningTicket(recipient2.address, sender, recipientRand, faceValue)
            const senderSig = await signMsg(getTicketHash(ticket), sender)
            const senderSig2 = await signMsg(getTicketHash(ticket2), sender)
            const fromBlock = (await web3.eth.getBlock("latest")).number
            ...
            const filter = await broker.filters.WinningTicketRedeemed(sender, recipient)
            const events = await broker.queryFilter(filter, fromBlock, "latest")
    ...
    describe("batchRedeemWinningTickets", () => {
        it("should fail if the system is paused", async () => {
            ...
            await expect(
                broker.connect(signers[1]).batchRedeemWinningTickets(
                    [ticket, ticket2],
        ...
        it("redeems 2 tickets from the same sender", async () => {
            ...
            await broker.connect(signers[1]).batchRedeemWinningTickets(
                [ticket, ticket2],
                [senderSig, senderSig2],
                [recipientRand, recipientRand]
            )
        ...
        it("redeems 2 tickets from different senders", async () => {
            ...
            let filter = await broker.filters.WinningTicketRedeemed(sender, recipient)
            const sender1Events = await broker.queryFilter(filter, fromBlock, "latest")
            filter = await broker.filters.WinningTicketRedeemed(sender2.address, recipient)
            const sender2Events = await broker.queryFilter(filter, fromBlock, "latest")
    ...
    describe("unlock", () => {
        ...
        it("reverts when both deposit and reserve are zero", async () => {
            await expect(broker.unlock()).to.be.revertedWith("sender deposit and reserve are zero")
        })
        ...
        it("reverts when called twice", async () => {
            ...
            await expect(broker.unlock()).to.be.revertedWith("unlock already initiated")
        })
        ...
        it("reverts when called twice by multiple senders", async () => {
            ...
            await expect(broker.unlock()).to.be.revertedWith("unlock already initiated")
            await expect(broker.connect(sender2).unlock()).to.be.revertedWith("unlock already initiated")
        })
    ...
    describe("cancelUnlock", () => {
        ...
        it("reverts if sender is not in an unlocking state", async () => {
            await expect(broker.cancelUnlock()).to.be.revertedWith("no unlock request in progress")
        })
        ...
        it("prevents withdrawal", async () => {
            ...
            await broker.cancelUnlock()
            await expect(broker.withdraw()).to.be.revertedWith("no unlock request in progress")
        })
    ...
    describe("withdraw", () => {
        ...
        it("reverts when both deposit and reserve are zero", async () => {
            await expect(broker.withdraw()).to.be.revertedWith("sender deposit and reserve are zero")
        })
        ...
        it("reverts when account is locked", async () => {
            ...
            await expect(broker.withdraw()).to.be.revertedWith("account is locked")
        })
        ...
        it("transfers the sum of deposit and reserve to sender", async () => {
            ...
            const events = await fixture.minter.queryFilter(filter, fromBlock, "latest")

            assert.equal(events.length, 1)
            const event = events[0]
            assert.equal(event.args.to, sender)
            assert.equal(event.args.amount.toString(), (deposit + reserve).toString())
        })
    ...
    describe("claimableReserve", () => {
        ...
        it("returns 0 if claimant is not active in the current round", async () => {
            const reserve = 1000
            await broker.fundReserve({value: reserve})
            assert.equal((await broker.claimableReserve(sender, constants.NULL_ADDRESS)).toString(), "0")
        })
