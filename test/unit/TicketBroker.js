...
describe("TicketBroker", () => {
    ...
    describe("redeemWinningTicket", () => {
        ...
        it("reverts if sender's deposit and reserve are zero", async () => {
            ...
            context("supported ticket signatures", () => {
                const recipientRand = 5
                const faceValue = 1000
                ...
                beforeEach(async () => {
                    ...
                    await token
                        .connect(transcoder)
                        .transfer(transcoder.address, amount)
                    ...
                    const creationRoundBlockHash =
                        await roundsManager.blockHash(block)
                    ...
                    if (blockNumber !== 0) {
                        ...
                        expect(
                            blockNumber,
                            "different block-number scenario"
                        ).to.be.eq(updatedBlockNumber)
                    }
                ...
                it("under the max `winProb` value, a ticket has two signature formats that are both eligible to win the lottery, but TicketBroker only accepts legacy long signatures", async () => {
                    ...
                    const brokerMock = await BrokerMock.deploy(
                        controller.address
                    )
                    ...
                    const eip2098SignatureToNumber =
                        await brokerMock.checkResult(
                            eip2098Signature,
                            recipientRand
                        )
                ...
                it("redeeming a redeemed ticket with an eip-2098 signature should always result in failure", async () => {
                    const deposit = (
                        await broker.getSenderInfo(broadcaster.address)
                    ).sender.deposit
