    ...
    describe("redeemWinningTicket", () => {
        ...
        describe("only legacy long-signatures are supported by the protocol ", () => {
            ...
            beforeEach(async () => {
                ...
                ticket = createWinningTicket(recipient, sender, recipientRand)
            })

            it("only accepts legacy long signatures", async () => {
                const brokerMock = await (
                    await ethers.getContractFactory("TickerBrokerExtendedMock")
                ).deploy(fixture.controller.address)
            ...
            it("redeeming a redeemed ticket with an eip-2098 signature should always result in failure", async () => {
                const deposit = (await broker.getSenderInfo(sender)).sender
                    .deposit
                ...
                await expect(
                    broker.redeemWinningTicket(ticket, sig2098, recipientRand)
                ).to.be.revertedWith("ticket is used")

                const endDeposit = (await broker.getSenderInfo(sender)).sender
                    .deposit
