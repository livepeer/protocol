...
describe("LIP36 transition", () => {
    ...
    before(async () => {
        ...
        const StakingManagerPreLIP36Fac = await ethers.getContractFactory("StakingManagerPreLIP36", {
            libraries: {
                SortedDoublyLL: ll.address
            }
        })
        stakingManagerFac = await ethers.getContractFactory("StakingManager", {
            libraries: {
                SortedDoublyLL: ll.address
            }
        })
        ...
        // deploy proxy
        proxy = await fixture.deployAndRegister(await ethers.getContractFactory("ManagerProxy"), "StakingManager", fixture.controller.address, contractId("StakingManager"))


        // deploy proxy target implementation
        await fixture.deployAndRegister(StakingManagerPreLIP36Fac, "StakingManager", fixture.controller.address)

        // bind ABI to proxy
        stakingManager = await ethers.getContractAt("StakingManagerPreLIP36", proxy.address)
    ...
    describe("pendingStake", async () => {
        ...
        beforeEach(async () => {
            ...
            await stakingManager.bond(1000, transcoder.address)
            await stakingManager.transcoder(50 * PERC_MULTIPLIER, 25 * PERC_MULTIPLIER)
            ...
            // deploy LIP-36
            await fixture.deployAndRegister(stakingManagerFac, "StakingManager", fixture.controller.address)
            stakingManager = await ethers.getContractAt("StakingManager", proxy.address)
        ...
        describe("delegator", () => {
            ...
            it("should return pending rewards if delegator has already claimed since LIP-36", async () => {
                ...
                await stakingManager.reward()
                assert.equal((await stakingManager.pendingStake(delegator.address, currentRound + 2)).toString(), (1000 + pendingRewards0 + pendingRewards1 + pendingRewards2).toString())
            })
        })
        ...
        describe("transcoder", () => {
            ...
            it("should return pending rewards for rounds both before and after LIP-36 combined when endRound == currentRound", async () => {
                ...
                assert.equal(
                    (await stakingManager.pendingStake(transcoder.address, currentRound + 1)).toString(),
                    1000 + pendingRewards0 + pendingRewards1 + cumulativeRewards,
                    "should return sum of bondedAmount and pending rewards as both a delegator and transcoder for 2 rounds"
                )
            })
    ...
    describe("pendingFees", () => {
        ...
        beforeEach(async () => {
            ...
            await fixture.ticketBroker.execute(
                ...
                functionEncodedABI(
                    "updateTranscoderWithFees(address,uint256)",
                    ["address", "uint256"],
                    [transcoder.address, 1000]
                )
            )
        ...
        describe("delegator", () => {
            it("should return pending fees for rounds both before and after LIP-36 combined when endRound < currentRound", async () => {
                ...
                assert.equal((await stakingManager.pendingFees(delegator.address, currentRound)).toString(), pendingFees0 + pendingFees1, "should return sum of collected fees and pending fees for 2 rounds")
            })
            ...
            it("should return pending fees for rounds both before and after LIP-36 combined when endRound == currentRound", async () => {
                ...
                const pendingFees1 = Math.floor((250 * (1250 * PERC_DIVISOR / 3000)) / PERC_DIVISOR)
                assert.equal(
                    (await stakingManager.pendingFees(delegator.address, currentRound + 1)).toNumber(),
        ...
        describe("transcoder", () => {
            it("should return pending fees for rounds both before and after LIP-36 combined when endRound < currentRound", async () => {
                ...
                assert.equal(
                    (await stakingManager.pendingFees(transcoder.address, currentRound )).toNumber(),
